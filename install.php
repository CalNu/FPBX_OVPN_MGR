...
$directories = [$baseDir, $pkiDir, "{$pkiDir}/private", "{$pkiDir}/issued", $tftpDir, "{$baseDir}/logs"];
...
// 2. Initialize 1024-bit Legacy PKI
...
// 4. Set Permissions
@exec("chown -R asterisk:asterisk " . escapeshellarg($baseDir) . " " . escapeshellarg($tftpDir) . " 2>&1");
@exec("chmod -R 775 " . escapeshellarg($baseDir) . " " . escapeshellarg($tftpDir) . " 2>&1");

// 5. Launch OpenVPN daemon safely
